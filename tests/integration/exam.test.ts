...
describe("GET /exam/filter", () => {
  ...
  describe("when token is valid", () => {
    it("should respond with status 404 when examId does not exist (= 0)", async () => {
      ...
      const response = await server.get("/exam/filter?search=0").set("Authorization", `Bearer ${token}`);
    ...
    it("should respond with status 404 when examId does not exist (> 1)", async () => {
      ...
      const response = await server.get("/exam/filter?search=1").set("Authorization", `Bearer ${token}`);
      expect(response.status).toEqual(httpStatus.NOT_FOUND);
    });
    ...
    it("should respond with status 404 when user is not exam owner", async () => {
      ...
      const response = await server.get(`/exam/filter?search=${exam.name}`).set("Authorization", `Bearer ${token}`);
      expect(response.status).toEqual(httpStatus.UNAUTHORIZED);
    });

    it("should respond with status 200 and exam data when there is an exam information for given filter - all letters", async () => {
      const user = await createUser();
      const exam = await createExamData(user);
      await createExamData(user);
      const token = await generateValidToken(user);
      const response = await server.get(`/exam/filter?search=${exam.name}`).set("Authorization", `Bearer ${token}`);
    ...
    it("should respond with status 200 and exam data when there is an exam information for given filter - first letters", async () => {
      const user = await createUser();
      const exam = await createExamData(user);
      await createExamData(user);
      const token = await generateValidToken(user);
      const filter = exam.name.slice(0, 3);
      const response = await server.get(`/exam/filter?search=${filter}`).set("Authorization", `Bearer ${token}`);
